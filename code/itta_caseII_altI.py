...
class RobotTrustModel(torch.nn.Module):
    ...
    def compute_trust(self, can_l_vec, can_u_vec, will, b_vec, p_vec):
...
if __name__ == "__main__":
    ...
    for iter in range(0, num_iter, 1):
        ...
        will_prog = [] #array for the progression of willingness
        ...
        for i in range(num_tasks): #iterates from 0 to num_tasks-1 for a total of num_tasks times
            ...
            robottrust_i = model.compute_trust(can_l_vec = robot_can_l, can_u_vec = robot_can_u, will = robot_w, b_vec = robot_beta, p_vec = p[:,i])
